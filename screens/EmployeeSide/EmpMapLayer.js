...
import { BASE_URL, MAP_URL, ICON_IMG_URL } from '../../Api/BaseConfig';
...
const EmpMapScreen = ({navigation}) => {
  ...
  const fetchData = async () => {
    ...
    console.log(user);
    try {
      const res = await layerApi.getAllUserLayerAssignments();
      const filteredLayers = res.filter(layer => layer.user_id === user.id).map((l) => ({
        id:l.layer_name,
        name:l.layer_name,
        category:l.layer_category,
        iconImg:l.image
    }));
      console.log(filteredLayers);
      setUserLayers(filteredLayers);
    } catch (err) {
      console.error("Failed to fetch user layers:", err);
      setError('Failed to load user layers');
    }
  };
        ...
        {searchQuery.length > 0 && (
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              >
                <MaterialIcons 
                // layerTypes.find(l => l.id === item.loc_type)?.icon ||
                  name={ 'place'} 
                  size={20} 
          ...
          <ScrollView>
            {userLayers.map((layer) => (
              ...
              >
                ...
                /> */}
                <Image
                  source={{ uri: `${ICON_IMG_URL}/${layer.iconImg}` }}
          ...
          <View style={styles.modalContent}>
            ...
            {selectedLocation?.type === 'threat' ? (
              ...
              <>
                <View style={styles.locationTypeBadge}>
                  <MaterialIcons 
                    name={userLayers.find(l => l.id === selectedLocation.loc_type)?.icon || 'place'} 
                    size={16} 
                    color={colors.white} 
                  />
                  <Text style={styles.locationTypeText}>
                    {userLayers.find(l => l.id === selectedLocation.loc_type)?.name || selectedLocation.loc_type}
                  </Text>
                </View>
                <Text style={styles.modalTitle}>{selectedLocation.name}</Text>
...
// import { BASE_URL, ICON_IMG_URL, MAP_URL } from '../../Api/BaseConfig';
// import layerApi from '../../Api/LayerApi';
// import Storage from '../../utils/localStorage';

// const EmpMapLayersScreen = () => {
//   // State declarations
//   const [userLayers, setUserLayers] = useState([]);
//   const [activeLayer, setActiveLayer] = useState(null);
...
//   // Replace your leafletHTML constant with this:
// const leafletHTML = `
// <!DOCTYPE html>
// <html>
//   <head>
...
//   <body>
//     <div id="map"></div>
//     <div style="position:absolute;top:0;left:0;color:white;background:black;padding:10px;z-index:1000;">
//   DEBUG: Map Container
// </div>
//     <script src="https://unpkg.com/leaflet@1.7.1/dist/leaflet.js"></script>
//     <script>
...
//                 icon: L.icon({
//                   iconUrl: loc.iconUrl || 'https://unpkg.com/leaflet@1.7.1/dist/images/marker-icon.png',
//                   iconSize: [38, 38],
//                   iconAnchor: [19, 38],
//                   popupAnchor: [0, -38]
//                 })
...
//               marker.bindPopup(
//                 \`<div>
//                   <h3 style="font-size:16px;font-weight:bold;margin:0 0 5px 0;">\${loc.name}</h3>
//                   <p style="font-size:14px;color:#666;margin:0;">\${loc.description}</p>
//                 </div>\`
//               );
...
//               polygon.bindPopup(
//                 \`<div>
//                   <h3 style="font-size:16px;font-weight:bold;margin:0 0 5px 0;">\${threat.threat_level}</h3>
//                   <p style="font-size:14px;color:#666;margin:0;">
//                     <span style="display:inline-block;width:14px;">⏱</span>
//                     \${threat.start_time} - \${threat.end_time}
//                   </p>
//                 </div>\`
//               );
...
//               const polyline = L.polyline(line.coordinates.map(coord => [coord[1], coord[0]]), {
//                 color: color,
//                 weight: 6,
//                 opacity: 0.8,
//                 dashArray: dashArray
...
//               polyline.bindPopup(
//                 \`<div>
//                   <h3 style="font-size:16px;font-weight:bold;margin:0 0 5px 0;">\${line.name}</h3>
//                   <p style="font-size:14px;color:#666;margin:0;">\${line.description}</p>
//                 </div>\`
//               );
...
//     try {
//       const res = await layerApi.getAllUserLayerAssignments();
//       const filteredLayers = res.filter(layer => layer.user_id === user.id);
//       console.log(filteredLayers);
//       setUserLayers(filteredLayers);
//     } catch (err) {
//       console.error("Failed to fetch user layers:", err);
...
//   // Fetch data when a layer is selected
//   useEffect(() => {
...
//       setLoading(true);
//       try {
//         const selected = userLayers.find(l => l.layer_name === activeLayer);
//         if (!selected) return;

//         // Clear previous data
//         setMapData({ locations: [], threats: [], lines: [] });

//         if (selected.layer_category === 'threat') {
//           const res = await axios.get(`${BASE_URL}/api/location/threat-simulation/all`);
//           const threats = res.data.data.map(threat => ({
//             ...threat,
//             path: threat.path,
//             start_time: formatTime(threat.start_time),
//             end_time: formatTime(threat.end_time)
//           }));
//           setMapData(prev => ({ ...prev, threats }));
//         } else if (selected.layer_category === 'line') {
//           const res = await axios.get(`${BASE_URL}/api/location/map-lines/all`);
//           setMapData(prev => ({ ...prev, lines: res.data }));
//         } else if (selected.layer_category === 'location') {
//           const res = await axios.post(`${BASE_URL}/api/location/map-locations`, {
//             type: selected.layer_name,
//           });
//           const locations = res.data.map(loc => ({
//             ...loc,
//             iconUrl: selected.image ? `${ICON_IMG_URL}/${selected.image}` : null
//           }));
//           setMapData(prev => ({ ...prev, locations }));
//         }
//       } catch (err) {
...
//   // Update WebView when mapData changes
//   useEffect(() => {
...
//   return (
//     <View style={styles.container}>
//       {/* Update your WebView component props:*/}
// <WebView
//     ref={webViewRef}
//     source={{ 
//       html: leafletHTML,
...
//     onLoadStart={() => console.log('MAP LOAD START')}
//     onLoadEnd={() => {
//       console.log('MAP LOAD END');
//     //   webViewRef.current.injectJavaScript(`
//     //     document.body.style.backgroundColor = 'red'; // Debug color
//     //     console.log('WebView dimensions:', document.body.offsetWidth, document.body.offsetHeight);
//     //     true;
//     //   `);
//     }}
...
//             <TouchableOpacity
//               key={layer.id}
//               onPress={() => setActiveLayer(layer.layer_name)}
//               style={[
//                 styles.layerItem,
//                 activeLayer === layer.layer_name && styles.activeLayerItem,
//               ]}
//             >
//               {layer.image ? (
//                 <Image
...
//                 />
//               ) : (
//                 <View style={styles.layerIconPlaceholder}>
//                   <Icon name="map" size={16} color="#666" />
//                 </View>
//               )}
//               <Text style={styles.layerName}>{layer.layer_name}</Text>
//             </TouchableOpacity>
//           ))}
//         </ScrollView>
//       </View>
...
//             <MaterialIcon name="close" size={20} color="#b91c1c" />
//           </TouchableOpacity>
//         </View>
//       )}
//     </View>
...
//     flex: 1,
//     position: 'relative',
//     backgroundColor: 'blue', // Temporary to verify container size
//   },
// //   map: {
// //     position: 'absolute',
// //     top: 0,
...
// //     backgroundColor: 'green', // Temporary debug color
// //   },
// // Replace map style with:
// map: {
//   width: '100%',
//   height: '100%',
...
// export default EmpMapLayersScreen;
