...
const ThreatLayerScreen = () => {
  ...
  const [isStartTimePickerVisible, setStartTimePickerVisibility] = useState(false);
  const [isEndTimePickerVisible, setEndTimePickerVisibility] = useState(false);
  const [threatLevels, setThreatLevels] = useState([]); // State for fetched threat levels
  const [selectedThreatLevel, setSelectedThreatLevel] = useState(''); // State for selected threat level
  const webViewRef = useRef(null);
  ...
  // Fetch threat levels from API
  useEffect(() => {
    const fetchThreatLevels = async () => {
      try {
        const response = await layerApi.getLayersByType('threat')
        const formattedLevels = response.map(item => ({
          key: item.id,
          value: item.name
        }));
        setThreatLevels(formattedLevels);
        
        // Set default threat level if available
        if (formattedLevels.length > 0) {
          setSelectedThreatLevel(formattedLevels[0].value);
          setFormData(prev => ({
            ...prev,
            threatLevel: formattedLevels[0].value
          }));
        }
      } catch (error) {
        console.error('Error fetching threat levels:', error);
        Alert.alert('Error', 'Failed to load threat levels');
      }
    };
  ...
  // Update formData when threat level selection changes
  useEffect(() => {
    if (selectedThreatLevel) {
      ...
      // Update polygon color when threat level changes
      if (points.length >= 3) {
        webViewRef.current.injectJavaScript(`
          updatePolygonColor('${selectedThreatLevel}');
          true;
        `);
      }
    }
      ...
      <body>
        ...
        <script>
          ...
          function drawPolygon() {
            ...
            if (points.length >= 3) {
              polygon = L.polygon(points, {
                color: threatColors['${selectedThreatLevel || 'Low'}'],
                fillOpacity: 0.4
              }).addTo(map);
            }
          }
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.formContainer} contentContainerStyle={styles.formContent}>
        ...
        <View style={styles.formGroup}>
          <Text style={styles.label}>Threat Level *</Text>
          <SelectList
            setSelected={(val) => setSelectedThreatLevel(val)}
            data={threatLevels}
            save="value"
            ...
            defaultOption={threatLevels.length > 0 ? threatLevels[0] : null}
          />
        </View>
